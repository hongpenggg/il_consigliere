...
drop policy if exists "Users can view own instance" on public.game_instances;
create policy "Users can view own instance"
  on public.game_instances for select
  using (auth.uid() = user_id);

drop policy if exists "Users can upsert own instance" on public.game_instances;
create policy "Users can upsert own instance"
  on public.game_instances for insert
  with check (auth.uid() = user_id);

drop policy if exists "Users can update own instance" on public.game_instances;
